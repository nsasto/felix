  ...
  describe('CopilotChatButton', () => {
    describe('Visibility', () => {
      it('renders when copilot is enabled', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByRole('button', { name: /felix copilot chat/i })).toBeInTheDocument();
        });
      });
      ...
      it('does not render when copilot is disabled', async () => {
        ...
        expect(screen.queryByRole('button', { name: /felix copilot chat/i })).not.toBeInTheDocument();
      });
    ...
    describe('Button Behavior', () => {
      it('calls onClick when clicked', async () => {
        ...
        fireEvent.click(screen.getByRole('button', { name: /felix copilot chat/i }));
      ...
      it('changes aria-label when panel is open vs closed', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByRole('button', { name: /open felix copilot chat/i })).toBeInTheDocument();
        });
        ...
        await waitFor(() => {
          expect(screen.getByRole('button', { name: /close felix copilot chat/i })).toBeInTheDocument();
        });
      });
    });
    ...
    describe('Unread Badge', () => {
      it('shows unread badge when unreadCount > 0 and panel is closed', async () => {
        renderWithTheme(
          <CopilotChatButton isOpen={false} onClick={() => {}} unreadCount={5} />
        );
      ...
      it('does not show unread badge when panel is open', async () => {
        renderWithTheme(
          <CopilotChatButton isOpen={true} onClick={() => {}} unreadCount={5} />
        );
      ...
      it('shows 99+ for counts over 99', async () => {
        renderWithTheme(
          <CopilotChatButton isOpen={false} onClick={() => {}} unreadCount={150} />
        );
      ...
      it('does not show badge when unreadCount is 0', async () => {
        renderWithTheme(
          <CopilotChatButton isOpen={false} onClick={() => {}} unreadCount={0} />
        );
  ...
  describe('CopilotChatPanel', () => {
    ...
    describe('Panel Open/Close', () => {
      it('renders when isOpen is true', () => {
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} isOpen={true} />
        );

        expect(screen.getByRole('dialog', { name: /felix copilot chat/i })).toBeInTheDocument();
        expect(screen.getByText('Felix Copilot')).toBeInTheDocument();
      });

      it('has correct opacity when closed', () => {
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} isOpen={false} />
        );

        const dialog = screen.getByRole('dialog', { name: /felix copilot chat/i });
    ...
    describe('Empty State', () => {
      it('shows empty state when no messages', () => {
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} messages={[]} />
        );
    ...
    describe('Message Rendering', () => {
      it('renders user messages aligned right with correct styling', () => {
        const messages: ChatMessage[] = [
          { id: '1', role: 'user', content: 'Hello copilot!', timestamp: new Date() },
        ];
      ...
      it('shows timestamps on messages', () => {
        ...
        const messages: ChatMessage[] = [
          { id: '1', role: 'user', content: 'Test message', timestamp: testDate },
        ];
...
Implementation details and technical considerations go here.`;
        ...
        const messages: ChatMessage[] = [
          { id: '1', role: 'assistant', content: specContent, timestamp: new Date() },
        ];

        renderWithTheme(
          <CopilotChatPanel {...defaultProps} messages={messages} isStreaming={false} />
        );
...
- [ ] Second criterion must be verified`;
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} messages={messages} onInsertSpec={handleInsertSpec} isStreaming={false} />
        );
    ...
    describe('Input Area', () => {
      it('shows input placeholder', () => {
        ...
        expect(screen.getByPlaceholderText(/Ask me anything or type \/draft/)).toBeInTheDocument();
      });

      it('updates input value on change', () => {
        const handleInputChange = vi.fn();
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} onInputChange={handleInputChange} />
        );
      ...
      it('calls onSendMessage when Enter is pressed', () => {
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} inputValue="Test" onSendMessage={handleSendMessage} />
        );
      ...
      it('disables send button when input is empty', () => {
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} inputValue="" />
        );
    ...
    describe('Streaming State', () => {
      ...
      it('calls onCancelStream when cancel button is clicked', () => {
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} isStreaming={true} onCancelStream={handleCancelStream} />
        );
    ...
    describe('Quick Actions', () => {
      ...
      it('calls onQuickDraft when Draft Spec is clicked', () => {
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} onQuickDraft={handleQuickDraft} />
        );
      ...
      it('calls onHelpCommand when Help is clicked', () => {
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} onHelpCommand={handleHelpCommand} />
        );
    ...
    describe('Clear History', () => {
      it('shows confirmation dialog when clear history is clicked', () => {
        ...
        expect(confirmSpy).toHaveBeenCalledWith('Clear all conversation history?');
      ...
      it('calls onClearHistory when confirmed', () => {
        ...
        renderWithTheme(
          <CopilotChatPanel {...defaultProps} onClearHistory={handleClearHistory} />
        );
  ...
  describe('CopilotAvatar', () => {
    ...
    it('renders at custom size', () => {
      renderWithTheme(
        <CopilotAvatar state="idle" size={64} />
      );
  ...
  describe('CopilotChat (Integration)', () => {
    describe('localStorage Persistence', () => {
      it('loads chat history from localStorage on mount', async () => {
        const savedMessages: ChatMessage[] = [
          { id: '1', role: 'user', content: 'Previous message', timestamp: new Date('2026-01-27T10:00:00') },
          { id: '2', role: 'assistant', content: 'Previous response', timestamp: new Date('2026-01-27T10:01:00') },
        ];
        
        localStorage.setItem(
          `felix_copilot_chat_${mockProjectId}`,
          JSON.stringify(savedMessages.map(m => ({ ...m, timestamp: m.timestamp.toISOString() })))
        );

        renderWithTheme(
          <CopilotChat projectId={mockProjectId} onInsertSpec={mockOnInsertSpec} />
        );
      ...
      it('saves chat history to localStorage when messages change', async () => {
        ...
        vi.mocked(felixApi.streamCopilotChat).mockReturnValue(mockStreamController);
        ...
        await act(async () => {
          ...
          mockStreamController._triggerEvent({ done: true, avatar_state: 'idle' });
        });

        // Check localStorage
        await waitFor(() => {
          const saved = localStorage.getItem(`felix_copilot_chat_${mockProjectId}`);
      ...
      it('maintains separate history for different projects', async () => {
        const projectAMessages = [
          { id: '1', role: 'user' as const, content: 'Project A message', timestamp: new Date().toISOString() },
        ];
        const projectBMessages = [
          { id: '2', role: 'user' as const, content: 'Project B message', timestamp: new Date().toISOString() },
        ];

        localStorage.setItem('felix_copilot_chat_project-a', JSON.stringify(projectAMessages));
        localStorage.setItem('felix_copilot_chat_project-b', JSON.stringify(projectBMessages));
        ...
        await waitFor(() => {
          ...
          expect(screen.queryByText('Project B message')).not.toBeInTheDocument();
        });
      });
    ...
    describe('Panel Toggle', () => {
      it('opens panel when button is clicked', async () => {
        ...
        expect(screen.queryByRole('dialog', { name: /felix copilot chat/i })).toHaveClass('opacity-0');
        ...
        await waitFor(() => {
          expect(screen.getByRole('dialog', { name: /felix copilot chat/i })).toHaveClass('opacity-100');
        });
      });
      ...
      it('closes panel when button is clicked again', async () => {
        ...
        await waitFor(() => {
          expect(screen.getByRole('dialog', { name: /felix copilot chat/i })).toHaveClass('opacity-0');
        });
      });
    });
