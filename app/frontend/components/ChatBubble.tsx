...
import { IconFelix, IconTerminal } from './Icons';
  ...
  return (
    <div className={`flex gap-4 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${
        isUser ? 'bg-slate-700' : 'bg-felix-600 shadow-lg shadow-felix-900/20'
      }`}>
        {isUser ? (
      ...
      <div className={`flex flex-col flex-1 min-w-0 ${isUser ? 'items-end' : 'items-start'}`}>
        <div className={`relative px-4 py-3 rounded-2xl text-sm leading-relaxed max-w-full ${
          ...
          <div className={`whitespace-pre-wrap ${!isUser ? 'font-sans' : 'font-mono text-xs'}`}>
            {message.text}
          </div>
              ...
              <div className="flex flex-wrap gap-2">
                {message.sources.map((source, idx) => (
                  <a 
                    ...
                    className="text-[10px] px-2 py-1 bg-slate-800/80 border border-slate-700/50 rounded hover:text-felix-400 hover:border-felix-500/30 transition-all flex items-center gap-1.5"
                  >
          ...
          {message.isThinking && (
            <div className="mt-2 flex gap-1">
              <div className="w-1.5 h-1.5 bg-felix-500 rounded-full animate-bounce"></div>
              <div className="w-1.5 h-1.5 bg-felix-500 rounded-full animate-bounce [animation-delay:-0.15s]"></div>
              <div className="w-1.5 h-1.5 bg-felix-500 rounded-full animate-bounce [animation-delay:-0.3s]"></div>
            </div>
          )}
        </div>
