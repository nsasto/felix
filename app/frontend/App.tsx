...
const App: React.FC = () => {
  ...
  const renderKanban = () => {
    ...
    return (
      <div className="flex-1 flex gap-6 p-8 overflow-x-auto custom-scrollbar" style={{ backgroundColor: 'var(--bg-deepest)' }}>
        {columns.map(col => (
          <div key={col.status} className="flex-shrink-0 w-80 flex flex-col gap-4">
            <div className="flex items-center justify-between px-2">
              <div className="flex items-center gap-2">
                ...
                }`} style={{ backgroundColor: col.status === 'backlog' ? 'var(--text-muted)' : undefined }} />
                <h3 className="text-xs font-bold uppercase tracking-widest" style={{ color: 'var(--text-tertiary)' }}>{col.label}</h3>
              </div>
              <span className="text-[10px] font-mono px-1.5 py-0.5 rounded" style={{ color: 'var(--text-muted)', backgroundColor: 'var(--bg-deep)' }}>
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded uppercase ${
                      ...
                      backgroundColor: task.priority === 'low' ? 'var(--bg-surface)' : undefined,
                      color: task.priority === 'low' ? 'var(--text-tertiary)' : undefined
                    }}>
                      {task.priority}
                    </span>
                    <button className="opacity-0 group-hover:opacity-100 p-1 rounded transition-opacity" style={{ backgroundColor: 'transparent' }}>
                      <IconPlus className="w-3 h-3" style={{ color: 'var(--text-muted)' }} />
                    </button>
                  </div>
                  <h4 className="text-sm font-semibold mb-1 group-hover:text-felix-400 transition-colors" style={{ color: 'var(--text-secondary)' }}>{task.title}</h4>
                  <p className="text-[11px] leading-relaxed mb-3 line-clamp-2" style={{ color: 'var(--text-muted)' }}>{task.description}</p>
                  <div className="flex flex-wrap gap-1.5">
                    {task.tags.map(tag => (
                      <span key={tag} className="text-[9px] font-mono border px-1 rounded transition-colors" style={{ color: 'var(--text-muted)', borderColor: 'var(--border-default)' }}>#{tag}</span>
                    ))}
                  </div>
                </div>
              ))}
              <button className="w-full py-2 border border-dashed rounded-xl text-[10px] transition-all flex items-center justify-center gap-2 group" style={{ borderColor: 'var(--border-default)', color: 'var(--text-muted)' }}>
  ...
  const renderCanvas = () => {
    return (
      <div className="flex-1 flex overflow-hidden" style={{ backgroundColor: 'var(--bg-base)' }}>
        <div className="flex-1 flex flex-col border-r" style={{ borderColor: 'var(--border-default)' }}>
          <div className="h-12 border-b flex items-center px-6 justify-between backdrop-blur" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-base)' }}>
             <div className="flex items-center gap-3">
               <IconFileCode className="w-4 h-4" style={{ color: 'var(--accent-primary)' }} />
               <span className="text-xs font-mono font-bold" style={{ color: 'var(--text-tertiary)' }}>workspace/felix-core/orchestrator.ts</span>
             </div>
             <div className="flex items-center gap-2">
...
export const executeTask = (taskId: string) => {
  ...
  const renderAssets = () => {
    return (
      <div className="flex-1 flex overflow-hidden" style={{ backgroundColor: 'var(--bg-base)' }}>
        {/* Sub-nav Panel */}
        <div className="w-64 border-r flex flex-col flex-shrink-0" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-deep)' }}>
           <div className="h-12 border-b flex items-center px-4" style={{ borderColor: 'var(--border-default)' }}>
              <span className="text-[10px] font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>Project Workspace</span>
           </div>
           <div className="p-3 space-y-1 overflow-y-auto custom-scrollbar">
              {assets.map(asset => (
                <button 
                  ...
                  style={{ color: selectedAssetId !== asset.id ? 'var(--text-muted)' : undefined }}
                >
              ...
              <button className="w-full flex items-center gap-2 px-3 py-2 rounded-xl text-xs border border-dashed mt-4 transition-all" style={{ color: 'var(--text-muted)', borderColor: 'var(--border-default)' }}>
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ backgroundColor: 'var(--bg-deep)' }}>
           <div className="h-12 border-b flex items-center px-4 justify-between backdrop-blur z-20 flex-shrink-0" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-base)' }}>
              <div className="flex items-center gap-4">
                <div className="flex border rounded-lg p-0.5 shadow-inner" style={{ backgroundColor: 'var(--bg-deep)', borderColor: 'var(--border-default)' }}>
                  <button 
                    onClick={() => setAssetViewMode('edit')}
                    className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all`}
                    style={{ 
                      backgroundColor: assetViewMode === 'edit' ? 'var(--bg-surface)' : 'transparent',
                      color: assetViewMode === 'edit' ? 'var(--accent-primary)' : 'var(--text-muted)'
                    }}
                  >
                    SOURCE
                  </button>
                  <button 
                    ...
                    className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all`}
                    style={{ 
                      backgroundColor: assetViewMode === 'split' ? 'var(--bg-surface)' : 'transparent',
                      color: assetViewMode === 'split' ? 'var(--accent-primary)' : 'var(--text-muted)'
                    }}
                  >
                    ORCHESTRATE
                  </button>
                  <button 
                    ...
                    className={`px-3 py-1 text-[10px] font-bold rounded-md transition-all`}
                    style={{ 
                      backgroundColor: assetViewMode === 'preview' ? 'var(--bg-surface)' : 'transparent',
                      color: assetViewMode === 'preview' ? 'var(--accent-primary)' : 'var(--text-muted)'
                    }}
                  >
                    PREVIEW
                  </button>
                ...
                {(assetViewMode === 'edit' || assetViewMode === 'split') && (
                  <div className="flex items-center gap-0.5 border-l pl-4" style={{ borderColor: 'var(--border-default)' }}>
                    <button onClick={() => insertFormatting('# ')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="H1">
                      <span className="font-bold text-xs">H1</span>
                    </button>
                    <button onClick={() => insertFormatting('## ')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="H2">
                      <span className="font-bold text-xs">H2</span>
                    </button>
                    <button onClick={() => insertFormatting('**', '**')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="Bold">
                      <span className="font-bold text-xs uppercase">B</span>
                    </button>
                    <button onClick={() => insertFormatting('*', '*')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="Italic">
                      <span className="italic text-xs font-serif font-bold uppercase">I</span>
                    </button>
                    <button onClick={() => insertFormatting('- ')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="List">
                    ...
                    <button onClick={() => insertFormatting('`', '`')} className="p-1.5 rounded-md transition-all" style={{ color: 'var(--text-muted)' }} title="Code">
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="h-4 w-px" style={{ backgroundColor: 'var(--border-default)' }}></div>
                <div className="flex items-center gap-2">
                   <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></div>
                   <span className="text-[10px] font-mono uppercase" style={{ color: 'var(--text-muted)' }}>{activeAsset.name}</span>
                </div>
              </div>
           </div>

           {/* Flexible Content Panels */}
           <div className={`flex-1 flex overflow-hidden ${assetViewMode === 'split' ? 'divide-x' : ''}`} style={{ borderColor: 'var(--border-muted)' }}>
              {(assetViewMode === 'edit' || assetViewMode === 'split') && (
                <div className="flex-1 flex flex-col min-w-0 relative h-full">
                  ...
                  />
                  {assetViewMode === 'edit' && (
                    <div className="absolute top-4 right-4 text-[9px] font-mono uppercase tracking-[0.2em] px-3 py-1 rounded-full border backdrop-blur" style={{ color: 'var(--text-faint)', backgroundColor: 'var(--bg-deep)', borderColor: 'var(--border-muted)' }}>Resource Source Editor</div>
                  )}
                </div>
              )}

              {(assetViewMode === 'preview' || assetViewMode === 'split') && (
                <div className="flex-1 flex flex-col min-w-0 h-full relative" style={{ backgroundColor: 'var(--bg-base)' }}>
                   <div className="flex-1 p-12 overflow-y-auto custom-scrollbar markdown-preview font-sans max-w-4xl mx-auto w-full">
                      <div dangerouslySetInnerHTML={{ __html: parsedHtml }} />
                      {!parsedHtml && (
                        <div className="flex flex-col items-center justify-center h-full gap-4" style={{ color: 'var(--text-faint)' }}>
                   ...
                   {assetViewMode === 'preview' && (
                    <div className="absolute top-4 right-4 text-[9px] font-mono uppercase tracking-[0.2em] px-3 py-1 rounded-full border backdrop-blur" style={{ color: 'var(--text-faint)', backgroundColor: 'var(--bg-deep)', borderColor: 'var(--border-muted)' }}>Live Visualization</div>
                   )}
                </div>
              )}
           </div>
  ...
  const renderProjects = () => {
    return (
      <div className="flex-1 flex overflow-hidden" style={{ backgroundColor: 'var(--bg-base)' }}>
        ...
        <div className="w-80 border-r flex flex-col flex-shrink-0" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-deep)' }}>
          <ProjectSelector
        ...
        <div className="flex-1 flex flex-col min-w-0" style={{ backgroundColor: 'var(--bg-deep)' }}>
          ...
          {selectedRunId && selectedProjectId ? (
            ...
            <>
              {/* Project header */}
              <div className="h-16 border-b flex items-center px-8 backdrop-blur" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-base)' }}>
                <div className="flex-1">
                  <h2 className="text-lg font-bold" style={{ color: 'var(--text-secondary)' }}>
                    {selectedProject.name || selectedProject.path.split(/[\\/]/).pop()}
                  </h2>
                  <p className="text-[10px] font-mono truncate max-w-lg" style={{ color: 'var(--text-muted)' }}>
                ...
                <div className="flex items-center gap-4">
                  {selectedProject.status && (
                    <span className={`text-[10px] font-bold px-2 py-1 rounded-lg uppercase ${
                      ...
                      backgroundColor: !['running', 'complete', 'blocked'].includes(selectedProject.status || '') ? 'var(--bg-surface)' : undefined,
                      color: !['running', 'complete', 'blocked'].includes(selectedProject.status || '') ? 'var(--text-tertiary)' : undefined
                    }}>
                      {selectedProject.status}
                    </span>
                  )}
              ...
              <div className="flex-1 p-8 overflow-y-auto custom-scrollbar">
                <div className="grid grid-cols-3 gap-6 mb-8">
                  {/* Specs card */}
                  <div className="border rounded-2xl p-6 hover:border-felix-600/40 transition-all" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-default)' }}>
                  ...
                  <div className="border rounded-2xl p-6 hover:border-felix-600/40 transition-all" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-default)' }}>
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      }`} style={{ backgroundColor: !selectedProject.has_plan ? 'var(--bg-surface)' : undefined }}>
                        <IconKanban className={`w-5 h-5 ${
                          selectedProject.has_plan ? 'text-emerald-400' : ''
                        }`} style={{ color: !selectedProject.has_plan ? 'var(--text-muted)' : undefined }} />
                      </div>
                      <div>
                  ...
                  <div className="border rounded-2xl p-6 hover:border-felix-600/40 transition-all" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-default)' }}>
                    <div className="flex items-center gap-3 mb-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        selectedProject.has_requirements ? 'bg-amber-500/10' : ''
                      }`} style={{ backgroundColor: !selectedProject.has_requirements ? 'var(--bg-surface)' : undefined }}>
                        <IconCpu className={`w-5 h-5 ${
                          selectedProject.has_requirements ? 'text-amber-400' : ''
                        }`} style={{ color: !selectedProject.has_requirements ? 'var(--text-muted)' : undefined }} />
                      </div>
                      <div>
                        <h3 className="text-sm font-bold" style={{ color: 'var(--text-secondary)' }}>
                ...
                {/* Quick actions */}
                <div className="border rounded-2xl p-6 mb-6" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-default)' }}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              <div className="w-20 h-20 rounded-3xl flex items-center justify-center mb-6" style={{ backgroundColor: 'var(--bg-surface)' }}>
                <IconFelix className="w-10 h-10" style={{ color: 'var(--text-faint)' }} />
              </div>
              <h2 className="text-lg font-bold mb-2" style={{ color: 'var(--text-tertiary)' }}>
                No Project Selected
              </h2>
              ...
              {backendStatus === 'disconnected' && (
                <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl px-4 py-3 text-xs text-amber-400">
                  ...
                  <code className="block mt-2 px-2 py-1 rounded text-amber-300" style={{ backgroundColor: 'var(--bg-deepest)' }}>
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden font-sans selection:bg-felix-500/30" style={{ backgroundColor: 'var(--bg-deepest)', color: 'var(--text-secondary)' }}>
      {/* Primary Orchestration Sidebar */}
      <aside className="w-16 border-r flex flex-col items-center py-6 gap-6 z-30 shadow-2xl flex-shrink-0" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-base)' }}>
        <div 
        ...
        <div className="flex-1 flex flex-col items-center gap-4 w-full px-2">
          ...
          <button 
            ...
            style={{ 
              backgroundColor: uiState === 'projects' ? 'var(--bg-surface)' : 'transparent',
              color: uiState === 'projects' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <button 
            ...
            style={{ 
              backgroundColor: uiState === 'kanban' ? 'var(--bg-surface)' : 'transparent',
              color: uiState === 'kanban' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <button 
            ...
            style={{ 
              backgroundColor: uiState === 'canvas' ? 'var(--bg-surface)' : 'transparent',
              color: uiState === 'canvas' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <button 
            ...
            style={{ 
              backgroundColor: uiState === 'assets' ? 'var(--bg-surface)' : 'transparent',
              color: uiState === 'assets' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <div className="h-px w-8 my-2" style={{ backgroundColor: 'var(--border-muted)' }}></div>
          <button className="p-3 transition-all w-full flex items-center justify-center rounded-2xl group" style={{ color: 'var(--text-faint)' }}>
        ...
        <div className="mt-auto flex flex-col items-center gap-4">
          ...
          <button 
            ...
            style={{ 
              backgroundColor: uiState === 'settings' ? 'var(--bg-surface)' : 'transparent',
              color: uiState === 'settings' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <div 
            ...
            style={{ backgroundColor: backendStatus === 'unknown' ? 'var(--text-muted)' : undefined }}
            title={`Backend: ${backendStatus}`}
          />
          <div className="w-9 h-9 rounded-2xl flex items-center justify-center text-[10px] font-bold border shadow-inner hover:border-felix-600/50 transition-colors cursor-pointer" style={{ backgroundColor: 'var(--bg-elevated)', borderColor: 'var(--border-default)', color: 'var(--text-muted)' }}>NS</div>
        </div>
      </aside>

      {/* Main View Container */}
      <div className="flex-1 flex flex-col relative min-w-0">
        <header className="h-14 border-b flex items-center px-8 justify-between backdrop-blur-2xl flex-shrink-0 z-10" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-deep)' }}>
           <div className="flex items-center gap-4">
              <h2 className="text-sm font-bold tracking-[0.15em] uppercase flex items-center gap-3" style={{ color: 'var(--text-primary)' }}>
                ...
                {uiState === 'config' && <div className="w-2 h-2 rounded-full shadow-lg" style={{ backgroundColor: 'var(--text-muted)' }}></div>}
              ...
              <div className="h-4 w-[1px] mx-2" style={{ backgroundColor: 'var(--border-default)' }}></div>
              <span className="text-[10px] font-mono truncate max-w-[300px] transition-colors cursor-default" style={{ color: 'var(--text-muted)' }}>
           ...
           <div className="flex items-center gap-6">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono font-bold uppercase tracking-tighter" style={{ color: 'var(--text-muted)' }}>
        ...
        {uiState === 'projects' ? renderProjects() : uiState === 'kanban' ? (
          selectedProjectId ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center" style={{ backgroundColor: 'var(--bg-deepest)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Select a project to view requirements</span>
              <button 
                onClick={() => setUiState('projects')}
          ...
          selectedProjectId ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center" style={{ backgroundColor: 'var(--bg-deepest)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Select a project to view specs</span>
              <button 
                onClick={() => setUiState('projects')}
                className="mt-4 px-4 py-2 text-xs font-bold text-felix-400 border border-felix-500/20 rounded-lg hover:bg-felix-500/10 transition-colors"
              >
          ...
          selectedProjectId ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center" style={{ backgroundColor: 'var(--bg-deepest)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Select a project to view configuration</span>
              <button 
                onClick={() => setUiState('projects')}
                className="mt-4 px-4 py-2 text-xs font-bold text-felix-400 border border-felix-500/20 rounded-lg hover:bg-felix-500/10 transition-colors"
              >
          ...
          selectedProjectId ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center" style={{ backgroundColor: 'var(--bg-deepest)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Select a project to view README</span>
              <button 
                onClick={() => setUiState('projects')}
                className="mt-4 px-4 py-2 text-xs font-bold text-felix-400 border border-felix-500/20 rounded-lg hover:bg-felix-500/10 transition-colors"
              >
          ...
          selectedProjectId ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center" style={{ backgroundColor: 'var(--bg-deepest)' }}>
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>Select a project to view settings</span>
              <button 
                onClick={() => setUiState('projects')}
                className="mt-4 px-4 py-2 text-xs font-bold text-felix-400 border border-felix-500/20 rounded-lg hover:bg-felix-500/10 transition-colors"
              >
      ...
      <footer className="h-8 border-t flex items-center px-6 justify-between text-[10px] font-mono z-40 fixed bottom-0 left-0 right-0 select-none flex-shrink-0 backdrop-blur-xl" style={{ borderColor: 'var(--border-default)', backgroundColor: 'var(--bg-base)', color: 'var(--text-muted)' }}>
         <div className="flex items-center gap-6">
            <div className="flex items-center gap-2 group cursor-default" style={{ color: 'var(--accent-primary)' }}>
            ...
            <div className="h-4 w-[1px] opacity-50" style={{ backgroundColor: 'var(--border-default)' }}></div>
            <span className="opacity-60 uppercase tracking-tighter" style={{ color: 'var(--text-faint)' }}>ID: FLX-ORCH-8821</span>
            <span className="opacity-60 uppercase" style={{ color: 'var(--text-faint)' }}>Load: 0.42 / 1.00</span>
         </div>
         <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 group cursor-pointer">
               ...
               <span className="uppercase tracking-[0.1em] transition-colors">Workspace Encrypted</span>
            </div>
            <div className="h-4 w-[1px] opacity-50" style={{ backgroundColor: 'var(--border-default)' }}></div>
            <span className="cursor-pointer transition-colors uppercase tracking-widest font-bold">UTF-8</span>
         </div>
      </footer>
    </div>
  );
