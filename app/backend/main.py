...
def migrate_agent_config():
        ...
        if config_data:
            ...
            if "name" in agent_config or "executable" in agent_config:
                ...
                default_agent = {
                    ...
                    "args": agent_config.get("args", ["exec", "--skip-permissions-unsafe"]),
                    "working_directory": agent_config.get("working_directory", "."),
                    "environment": agent_config.get("environment", {})
                }
                logger.info(f"Migrating legacy agent '{default_agent['name']}' to agents.json as ID 0")
    ...
    if config_data:
        ...
        if has_legacy_fields and not has_agent_id:
            ...
            logger.info("Migrating config.json from legacy inline agent to agent_id reference")
