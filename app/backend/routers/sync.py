...
async def verify_api_key(
    ...
    try:
        ...
        if is_database_connection_error(e):
            log_sync_error("Database unavailable during API key verification", exc=e)
            raise HTTPException(
                status_code=503,
                detail="Database temporarily unavailable. Please retry.",
            )
        log_sync_error("Error during API key verification", exc=e)
        raise HTTPException(
            status_code=500, detail=f"Error during API key verification: {str(e)}"
        )
...
class RunCreate(BaseModel):
    ...
    id: Optional[str] = Field(
        None, description="Optional run ID (UUID will be generated if not provided)"
    )
    ...
    phase: Optional[str] = Field(
        None, description="Run phase (e.g., 'planning', 'building')"
    )
...
class RunEvent(BaseModel):
    ...
    level: str = Field(
        ..., description="Event level ('info', 'warn', 'error', 'debug')"
    )
...
class RunCompletion(BaseModel):
    ...
    status: str = Field(
        ..., description="Final run status ('completed', 'failed', etc.)"
    )
    ...
    summary_json: Optional[Dict[str, Any]] = Field(
        None, description="Optional summary data as JSON"
    )
...
class FileUploadResult(BaseModel):
    ...
    reason: Optional[str] = Field(
        None, description="Reason for status (e.g., 'unchanged' for skipped)"
    )
...
async def create_run(
    ...
    try:
        ...
        agent = await db.fetch_one(
            "SELECT id FROM agents WHERE id = :agent_id", {"agent_id": body.agent_id}
        )
        if not agent:
            raise HTTPException(
        ...
        if is_database_connection_error(e):
            raise_database_unavailable(
                "run creation", e, agent_id=body.agent_id, project_id=body.project_id
            )
...
async def append_events(
    ...
    try:
        ...
        log_sync_debug(
            f"Appended {len(events)} events", run_id=run_id, event_count=len(events)
        )
        ...
        if is_database_connection_error(e):
            raise_database_unavailable(
                "event append", e, run_id=run_id, event_count=len(events)
            )
        # Log and return 500 for other database errors
        log_sync_error(
            "Failed to append events", run_id=run_id, event_count=len(events), exc=e
        )
        raise HTTPException(
            status_code=500, detail=f"Database error during event append: {str(e)}"
        )
    ...
    try:
        ...
        await db.execute(
            update_query,
            {
                ...
                "summary_json": (
                    json.dumps(body.summary_json) if body.summary_json else None
                ),
            },
        )
        ...
        event_type = (
            "completed" if body.status in ("completed", "succeeded") else "failed"
        )
        ...
        if is_database_connection_error(e):
            raise_database_unavailable(
                "run completion", e, run_id=run_id, status=body.status
            )
        # Log and return 500 for other database errors
        log_sync_error("Failed to finish run", run_id=run_id, status=body.status, exc=e)
        raise HTTPException(
            status_code=500, detail=f"Database error during run completion: {str(e)}"
        )
...
def validate_file_path(path: str, *, run_id: Optional[str] = None) -> None:
    ...
    if not is_safe_file_path(path):
        log_sync_warning(f"Rejected unsafe file path: {path}", run_id=run_id, path=path)
        raise HTTPException(
            status_code=400,
...
async def upload_files(
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400, detail=f"Invalid manifest JSON: {str(e)}"
            )
        ...
        total_bytes_uploaded = sum(
            r.size_bytes for r in results if r.status == "uploaded"
        )
        ...
        if elapsed_sec > target_sec:
            throughput_mbps = (
                (total_bytes_uploaded / (1024 * 1024)) / elapsed_sec
                if elapsed_sec > 0
                else 0
            )
            log_sync_warning(
        ...
        # Check for database connection errors - return 503 for transient failures
        if is_database_connection_error(e):
            raise_database_unavailable("file upload", e, run_id=run_id)
        # Check for storage errors - return 503 for transient failures
        if is_storage_error(e):
            raise_storage_unavailable("file upload", e, run_id=run_id)
        # Log and return 500 for other errors
        log_sync_error("Failed to upload files", run_id=run_id, exc=e)
        raise HTTPException(
...
async def list_files(
    ...
    try:
        ...
        # Check for database connection errors - return 503 for transient failures
        if is_database_connection_error(e):
            raise_database_unavailable("file listing", e, run_id=run_id)
        # Log and return 500 for other errors
        log_sync_error("Failed to list files", run_id=run_id, exc=e)
        raise HTTPException(
            status_code=500, detail=f"Database error during file listing: {str(e)}"
        )
...
async def download_file(
    ...
    # Log API key usage for audit trail
    await log_api_key_usage(
        db, api_key, f"/api/runs/{run_id}/files/{file_path}", http_request
    )
    ...
    try:
        ...
        if not file_record:
            raise HTTPException(
                status_code=404, detail=f"File not found in database: {file_path}"
            )
        ...
        if not await storage.exists(storage_key):
            raise HTTPException(
                status_code=404, detail=f"File not found in storage: {file_path}"
            )
        ...
        if is_database_connection_error(e):
            raise_database_unavailable(
                "file download", e, run_id=run_id, file_path=file_path
            )
        # Check for storage errors - return 503 for transient failures
        if is_storage_error(e):
            raise_storage_unavailable(
                "file download", e, run_id=run_id, storage_key=file_path
            )
        # Log and return 500 for other errors
        log_sync_error(
        ...
        raise HTTPException(
            status_code=500, detail=f"Error during file download: {str(e)}"
        )
...
async def list_events(
    ...
    after: Optional[int] = Query(
        None, description="Cursor for pagination - return events after this ID"
    ),
    limit: int = Query(
    ...
    try:
        ...
        # Check for database connection errors - return 503 for transient failures
        if is_database_connection_error(e):
            raise_database_unavailable("event listing", e, run_id=run_id)
        # Log and return 500 for other errors
        log_sync_error("Failed to list events", run_id=run_id, exc=e)
        raise HTTPException(
            status_code=500, detail=f"Database error during event listing: {str(e)}"
        )
