...
class CopilotChatRequest(BaseModel):
    ...
    project_path: Optional[str] = Field(
        None, description="Project path for context loading"
    )
...
async def verify_openai_connection(
    ...
    try:
        ...
        print(f"OpenAI auth error: {str(e)}")
        return False, "Invalid API key"
    except RateLimitError as e:
        print(f"OpenAI rate limit: {str(e)}")
        return True, None
...
async def verify_anthropic_connection(
    api_key: str, model: str
) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        print(f"Anthropic auth error: {str(e)}")
        return False, "Invalid API key"
    except RateLimitError as e:
        print(f"Anthropic rate limit: {str(e)}")
        return True, None
    except APIConnectionError as e:
        print(f"Anthropic connection error: {str(e)}")
        return False, f"Connection failed: {str(e)}"
    except APITimeoutError as e:
        print(f"Anthropic timeout: {str(e)}")
        return False, "Connection timeout - check your network"
    except Exception as e:
        print(f"Anthropic unexpected error: {type(e).__name__}: {str(e)}")
        error_str = str(e).lower()
        if "model" in error_str and (
            "not found" in error_str or "does not exist" in error_str
        ):
...
async def test_copilot_connection(
    ...
    if provider == "openai":
        ...
        return CopilotTestResult(
            success=True, error=None, provider=provider, model=model
        )
    else:
        return CopilotTestResult(
    ...
    return CopilotTestResult(
        success=success, error=error, provider=provider, model=model
    )
...
def build_messages(
    system_prompt: str, history: List[ChatMessage], user_message: str
) -> list:
    ...
    async def event_generator() -> AsyncGenerator[str, None]:
        try:
            ...
            service = create_copilot_service_from_config(
                config.copilot, api_key=api_key
            )
