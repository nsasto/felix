...
class TestVerifyApiKey:
    ...
    async def test_accepts_key_with_future_expiration(self):
        ...
        result = await verify_api_key(
            authorization="Bearer valid_future_key", db=fake_db
        )
    ...
    async def test_returns_api_key_info_with_all_fields(self):
        ...
        result = await verify_api_key(authorization="Bearer project_key", db=fake_db)

        assert isinstance(result, ApiKeyInfo)
        assert result.key_id == "key-006"
        assert result.project_id == "proj-006"
        assert result.name == "Project Key"
    ...
    async def test_raises_503_on_database_connection_error(self):
        ...
        fake_db = FakeDatabase(
            fetch_error=ConnectionRefusedError("Database unavailable")
        )
...
class TestLogApiKeyUsage:
    ...
    async def test_inserts_usage_log_record(self):
        ...
        api_key = ApiKeyInfo(key_id="key-001", project_id="proj-001", name="Test Key")
        ...
        insert_queries = [
            q
            for q, v in fake_db.executed_queries
            if "INSERT INTO api_key_usage_log" in q
        ]
    ...
    async def test_logs_client_ip_from_request(self):
        ...
        api_key = ApiKeyInfo(key_id="key-001", project_id="proj-001", name="Test")
...
class TestApiKeyIntegration:
    ...
    def test_sync_endpoint_accepts_valid_api_key(self, client):
        ...
        response = client.post(
            "/api/runs",
            json={"agent_id": "agent-001", "project_id": "project-001"},
