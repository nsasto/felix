...
@pytest.fixture
def mock_auth():
    with patch("routers.agent_configs.get_current_user", return_value={"user_id": "dev-user", "org_id": "org-1"}):
        yield
...
def mock_db():
    ...
    with patch("routers.agent_configs.get_db", return_value=mock_database):
        yield mock_database
...
class TestGetAgentConfigs:
    def test_get_agents_returns_profiles_and_active(self, client, mock_auth, mock_db, mock_repo):
...
class TestGetSingleAgentConfig:
    def test_get_agent_returns_404_when_missing(self, client, mock_auth, mock_db, mock_repo):
...
class TestCreateAgentConfig:
    def test_create_agent_sets_active_when_missing(self, client, mock_auth, mock_db, mock_repo):
...
class TestUpdateAgentConfig:
    def test_update_agent_uses_repo(self, client, mock_auth, mock_db, mock_repo):
...
class TestDeleteAgentConfig:
    def test_delete_active_agent_clears_active(self, client, mock_auth, mock_db, mock_repo):
...
class TestSetActiveAgent:
    def test_set_active_agent_updates_org_metadata(self, client, mock_auth, mock_db, mock_repo):
...
class TestGetActiveAgent:
    def test_get_active_agent_returns_profile(self, client, mock_auth, mock_db, mock_repo):
        mock_repo.get_by_id = AsyncMock(return_value=_profile("profile-1", "primary"))
