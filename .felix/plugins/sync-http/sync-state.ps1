...
function Start-EventFlushTimer {
    ...
    # Capture path before entering action block ($PSScriptRoot is empty inside -Action scriptblocks)
    $_syncStatePath = Join-Path $PSScriptRoot "sync-state.ps1"

    # Register event handler
    Register-ObjectEvent -InputObject $Global:HttpSyncState.FlushTimer -EventName Elapsed -MessageData $_syncStatePath -Action {
        try {
            . $Event.MessageData   # dot-source to define Invoke-EventFlush in this scope
            Invoke-EventFlush
        }
        catch {
