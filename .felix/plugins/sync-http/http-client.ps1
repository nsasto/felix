...
class HttpSync : IRunReporter {
    ...
    [hashtable] RegisterAgent([hashtable]$agentInfo) {
        # Real-time presence signal - not queued for retry.
        # Returns @{Success; Error} so callers can surface backend error detail.
        try {
            $result = $this.SendJsonRequestWithStatus("POST", "/api/agents/register-sync", $agentInfo)
            if ($result.Success) {
                $this.WriteLog("INFO", "Agent registered with backend")
                return @{ Success = $true; Error = $null }
            }
            return @{ Success = $false; Error = $result.Error }
        }
        catch {
            $this.WriteLog("DEBUG", "Agent registration failed (backend unreachable): $_")
    ...
    [void] AppendEvent([hashtable]$eventData) {
        try {
            ...
            $this.AppendToRunOutbox($runId, $eventData)
        }
        catch {
            $errorMsg = $_.Exception.Message
    ...
    # Map Felix internal event format to RunEvent schema expected by backend:
    # Felix: { type, timestamp, data: { level, message, ... } }
    # RunEvent: { type, level, message, payload }
    hidden [hashtable] ConvertToRunEvent([object]$event) {
        $eventHash = if ($event -is [hashtable]) { $event } else {
            $h = @{}
            $event.PSObject.Properties | ForEach-Object { $h[$_.Name] = $_.Value }
            $h
        }
        ...
        $level = if ($dataHash["level"]) { $dataHash["level"] } `
            elseif ($eventHash["level"]) { $eventHash["level"] } `
            else { "info" }

        $message = if ($dataHash["message"]) { $dataHash["message"] } `
            elseif ($eventHash["message"]) { $eventHash["message"] } `
            else { $null }
    ...
    # Send a batch of queued events directly to the backend.
    # Called by Invoke-EventFlush every 5 seconds and for critical events.
    # Falls back to outbox on network failure so events are persisted for retry.
    [void] AppendEvents([string]$runId, [array]$events) {
        if ($events.Count -eq 0) { return }

        try {
            $runEvents = @($events | ForEach-Object { $this.ConvertToRunEvent($_) })

            $success = $this.SendJsonRequest("POST", "/api/runs/$runId/events", $runEvents)

            if (-not $success) {
                # Fall through to outbox fallback below
                throw "SendJsonRequest returned false"
            }

            $this.WriteLog("INFO", "Sent $($events.Count) events for run $runId")
        }
        catch {
            $this.WriteLog("WARNING", "AppendEvents failed, persisting to outbox: $_")
            # Write each event to outbox so FlushRunEvents can retry at completion
            foreach ($ev in $events) {
                try { $this.AppendToRunOutbox($runId, $ev) } catch {}
            }
        }
    }

    [void] UpdateAgentStatus([string]$agentId, [string]$status) {
        # Best-effort - does not block run execution if this fails
        try {
            $this.SendJsonRequest("POST", "/api/agents/$agentId/status", @{ status = $status }) | Out-Null
            $this.WriteLog("INFO", "Agent $agentId status -> $status")
        }
        catch {
            $this.WriteLog("WARNING", "Failed to update agent status to $status`: $_")
        }
    }
    ...
    [void] UploadRunFolder([string]$runId, [string]$runFolderPath) {
        try {
            ...
            foreach ($pattern in $standardArtifacts) {
                $foundFiles = Get-ChildItem -Path $runFolderPath -Filter $pattern -File -ErrorAction SilentlyContinue
                foreach ($file in $foundFiles) {
                    $relativePath = $file.Name
                    $sha256 = $this.CalculateSHA256($file.FullName)
    ...
    hidden [void] AppendToRunOutbox([string]$runId, [hashtable]$eventData) {
        try {
    ...
    hidden [void] FlushRunEvents([string]$runId) {
        try {
            ...
            if ($events.Count -gt 0) {
                # Send events batch - backend expects a raw JSON array of RunEvent objects
    ...
    hidden [hashtable] SendJsonRequestWithStatus([string]$method, [string]$endpoint, [object]$body) {
        ...
        try {
            ...
            # Note: registration failures are handled by the caller (felix-agent.ps1 exits on failure when sync is enabled)
