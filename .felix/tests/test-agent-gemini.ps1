# Test Gemini Agent CLI
# Verifies that Gemini CLI is installed and can execute basic prompts
...
if (-not $cmd) {
    ...
    Write-Host "  Install from: https://ai.google.dev/gemini-api/docs/cli" -ForegroundColor Gray
    Write-Host ""
    exit 2
}
...
if (Test-Path $agentsFile) {
    try {
        ...
        $geminiAgent = $agentsConfig.agents | Where-Object { $_.adapter -eq "gemini" } | Select-Object -First 1

        if ($geminiAgent) {
            . "$PSScriptRoot/../core/agent-adapters.ps1"
            $adapter = Get-AgentAdapter -AdapterType "gemini"
            $args = $adapter.BuildArgs($geminiAgent, $false)
            Write-Host "  Using adapter args" -ForegroundColor Green
        }
        else {
            Write-Host "  No Gemini config found, using defaults" -ForegroundColor Yellow
...
if (-not $completed) {
    ...
    Write-Host "  Try running: gemini auth login" -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
