...
try {
    ...
    # Minimal prompts (content doesn't matter for invocation test; only that files exist)
    ...
    # Init git repo because felix-agent.ps1 calls git commands
    ...
    # Find newest run output.log and assert shim markers
    ...
    Assert-True ($output -match "(?m)^__AGENT_PROMPT_LEN__=") "Prompt length marker not found"

    Write-Host "PASS: Felix invoked agent with correct cwd/env"
}
finally {
