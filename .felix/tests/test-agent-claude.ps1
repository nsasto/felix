# Test Claude Agent CLI
# Verifies that Claude CLI is installed and can execute basic prompts
...
if (-not $cmd) {
    ...
    Write-Host "  Install from: https://docs.anthropic.com/claude/docs/cli" -ForegroundColor Gray
    Write-Host ""
    exit 2
}
...
if (Test-Path $agentsFile) {
    try {
        ...
        $claudeAgent = $agentsConfig.agents | Where-Object { $_.adapter -eq "claude" } | Select-Object -First 1

        if ($claudeAgent) {
            . "$PSScriptRoot/../core/agent-adapters.ps1"
            $adapter = Get-AgentAdapter -AdapterType "claude"
            $args = $adapter.BuildArgs($claudeAgent, $false)
            Write-Host "  Using adapter args" -ForegroundColor Green
        }
        else {
            Write-Host "  No Claude config found, using defaults" -ForegroundColor Yellow
...
if (-not $completed) {
    ...
    Write-Host "  Try running: claude auth login" -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
