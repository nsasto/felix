# Test Codex Agent CLI
# Verifies that Codex CLI is installed and can execute basic prompts
...
if (-not $cmd) {
    ...
    Write-Host "  Install from: https://github.com/openai/codex-cli" -ForegroundColor Gray
    Write-Host ""
    exit 2
}
...
if (Test-Path $agentsFile) {
    try {
        ...
        $codexAgent = $agentsConfig.agents | Where-Object { $_.adapter -eq "codex" } | Select-Object -First 1

        if ($codexAgent) {
            . "$PSScriptRoot/../core/agent-adapters.ps1"
            $adapter = Get-AgentAdapter -AdapterType "codex"
            $args = $adapter.BuildArgs($codexAgent, $false)
            Write-Host "  Using adapter args" -ForegroundColor Green
        }
        else {
            Write-Host "  No Codex config found, using defaults" -ForegroundColor Yellow
            $args = @("exec", "-C", ".", "-s", "danger-full-access", "--color", "never", "-")
        }
    }
    catch {
...
if (-not $completed) {
    ...
    Write-Host "  Try running: codex auth login" -ForegroundColor Yellow
    Write-Host ""
    exit 1
}
