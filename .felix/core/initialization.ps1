...
function Get-CurrentRequirement {
    ...
    param(
        ...
        # When true, skip the local "already complete" check.
        # Use when the server is the source of truth (e.g. --sync + server-assigned ID).
        [Parameter(Mandatory = $false)]
        [switch]$TrustServerStatus
    )
    ...
    try {
        ...
        if ($RequirementId) {
            ...
            # Skip this check when the server is authoritative (sync mode with assigned ID)
            if (-not $TrustServerStatus -and $currentReq.status -in @("complete", "done")) {
...
function Register-FelixAgent {
    ...
    $registrationSucceeded = Register-Agent -AgentId $AgentConfig.key -AgentName $AgentConfig.name -ProcessId $PID -Hostname $env:COMPUTERNAME -BackendBaseUrl $BackendBaseUrl
    
    # Start heartbeat background job if registration succeeded
    if ($registrationSucceeded) {
        # Note: Requires Start-HeartbeatJob from agent-registration.ps1
        $heartbeatJob = Start-HeartbeatJob -AgentId $AgentConfig.key -BackendBaseUrl $BackendBaseUrl -ApiKey $ApiKey -GitUrl $GitUrl
