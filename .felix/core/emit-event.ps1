...
function Emit-Log {
    ...
    # When running interactively (rich mode) write coloured text instead of NDJSON
...
function Emit-CommitStarted {
    <#
    .SYNOPSIS
    Emits commit_started event

    .PARAMETER Message
    Commit message (may be truncated for display)

    .PARAMETER Branch
    Git branch being committed to
    ...
    Emit-Event -EventType "commit_started" -Data $data
}

function Emit-CommitCompleted {
    <#
    .SYNOPSIS
    Emits commit_completed event
    ...
    $data = @{
        message = $Message
        pushed  = $Pushed.IsPresent
    }
    if ($Sha) { $data.sha = $Sha }
    if ($Branch) { $data.branch = $Branch }
    if ($FilesChanged -gt 0) { $data.files_changed = $FilesChanged }

    Emit-Event -EventType "commit_completed" -Data $data
}
