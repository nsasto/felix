    ...
    try {
        foreach ($cmd in $commands) {
            ...
            try {
                ...
                # If command starts with 'pytest' and pytest isn't in PATH, fall back to 'python -m pytest'
                $resolvedCommand = $cmd.command
                if ($resolvedCommand -match '^pytest(\s|$)') {
                    if (-not (Get-Command pytest -ErrorAction SilentlyContinue)) {
                        $resolvedCommand = $resolvedCommand -replace '^pytest', 'python -m pytest'
                        Emit-Log -Level "info" -Message "pytest not in PATH, falling back to: $resolvedCommand" -Component "validation"
                    }
                }
                $scriptBlock = [scriptblock]::Create($resolvedCommand)
                ...
                # Exit code 5 for pytest-based tests means "no tests found" - treat as success
                $isBackendTest = $cmd.command -match 'test-backend|pytest'
