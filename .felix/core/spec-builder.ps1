...
function Add-RequirementToJson {
    ...
    try {
        ...
        # Add new requirement (slim index — priority/tags/depends_on live in .meta.json sidecar)
        $specRelPath = $SpecPath -replace [regex]::Escape((Split-Path $RequirementsFile -Parent | Split-Path -Parent) + [IO.Path]::DirectorySeparatorChar), ''
        $specRelPath = $specRelPath -replace '\\', '/'
        $newRequirement = @{
            id                 = $RequirementId
            title              = $title
            status             = "planned"
        ...
        # Write .meta.json sidecar alongside the spec file
        $metaPath = $SpecPath -replace '\.md$', '.meta.json'
        $meta = [ordered]@{
            priority   = "medium"
            tags       = @()
            depends_on = @()
        ...
        Emit-Log -Level "debug" -Message "Written sidecar: $metaPath" -Component "spec-builder"
