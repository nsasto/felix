...
function Invoke-ContextBuilder {
    ...
    # LEARNINGS.md
    $learningsPath = Join-Path $ProjectPath "LEARNINGS.md"
    if (Test-Path $learningsPath) {
        $learnings = Get-Content $learningsPath -Raw -ErrorAction SilentlyContinue
        if ($learnings) {
            if ($learnings.Length -gt 1500) {
                $learnings = $learnings.Substring(0, 1500) + "`n`n[... truncated ...]"
            }
            $contextParts += "`n## LEARNINGS.md`n`n$learnings`n"
        }
    }
...
function Invoke-AgentForContextBuild {
    ...
    $agentProfile = $agentsConfig.agents | Where-Object { $_.id -eq $agentId }
