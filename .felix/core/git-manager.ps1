...
function Invoke-GitCommit {
    ...
    # Emit commit_started before writing to git
    $currentBranch = git rev-parse --abbrev-ref HEAD 2>$null
    if (Get-Command Emit-CommitStarted -ErrorAction SilentlyContinue) {
        Emit-CommitStarted -Message $finalMessage -Branch $currentBranch -Push:$Push
    }
    ...
    # Capture commit metadata for event
    $commitSha = git rev-parse HEAD 2>$null
    $filesChanged = (git diff-tree --no-commit-id -r --name-only HEAD 2>$null | Measure-Object -Line).Lines
    ...
    if (Get-Command Emit-CommitCompleted -ErrorAction SilentlyContinue) {
        Emit-CommitCompleted -Sha $commitSha -Message $finalMessage -Branch $currentBranch -FilesChanged $filesChanged -Pushed:$Push
    }
