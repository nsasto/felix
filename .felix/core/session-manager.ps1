    ...
    try {
        ...
        if ($proc) {
            ...
            if ($IsWindows -or $PSVersionTable.PSVersion.Major -le 5) {
                ...
                # Ignore exit code - taskkill reports errors for child processes that
                # are already dying, but the parent is what matters.
                try {
                    & taskkill /F /T /PID $session.pid 2>&1 | Out-Null
                }
                catch { <# ignore - parent death check below is authoritative #> }
            }
            else {
            ...
            # Verify the parent process is gone - child failures from taskkill are non-fatal
            $proc = Get-Process -Id $session.pid -ErrorAction SilentlyContinue
            if ($proc) {
                Write-Error "Failed to stop session: process $($session.pid) is still running"
                return $false
            }
        }
