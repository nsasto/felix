...
| `setup.ps1`    | `Invoke-Setup`                                                                  | Scaffolds `.felix/` (`requirements.json`, `state.json`, `config.json`, `model-pricing.json.example`, `.gitignore`) plus `specs/` and `runs/`, then runs the interactive sync / API-key wizard. Scaffold step is a no-op if `.felix/` already exists. |
...
| `agent-runner.ps1`   | `Invoke-AgentExecution`, `Write-AgentUsageArtifact`, `Test-AndEnforcePlanningGuardrails`                                    | Streams the LLM CLI subprocess, captures output, writes `runs/<run-id>/usage.json`, emits `llm_usage`, and enforces planning-mode guardrails. |
...
| `metrics-collector/` | Legacy / disabled                                                | Superseded for token/model accounting by core `agent-runner.ps1` usage artifacts and `felix query usage`. |
