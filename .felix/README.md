...
├── felix.ps1                # CLI dispatcher (PowerShell)
├── felix-cli.ps1            # PS wrapper used by felix.ps1
├── version.txt              # Engine version sentinel (embedded in felix.exe for update checks)
├── config.json              # Per-project runtime config (not committed — gitignored)
├── config.json.example      # Config example
├── config.md                # Configuration documentation
├── agents.json              # CLI agent overrides (local only)
├── agent-templates.json     # Agent templates used by setup
├── agent-models.json        # Model catalog for setup
├── requirements.json        # Slim requirement index (id, title, spec_path, status, commit_on_complete)
├── state.json               # Runtime execution state
├── outbox/                  # Sync queue (jsonl artifacts)
├── sync.log                 # Sync plugin log (rotating)
├── bin/                     # Embedded CLI assets
├── cli/                     # C# CLI assets
├── commands/                # PowerShell command handlers
...
│   ├── work-selector.ps1    # Work selection: local (requirements.json) or remote (GET /api/sync/work/next)
...
### Global Install (`felix install`)

The C# binary (`felix.exe`) embeds the entire engine as a zip. Running `felix install` extracts it to:

```
%LOCALAPPDATA%\Programs\Felix\
├── felix.ps1
├── felix-agent.ps1
├── felix-loop.ps1
├── version.txt
...
The installer also adds this directory to the user `PATH`. After install, `felix run S-0001` works from any directory.

### Project-Local `.felix/`

When running against a project, the engine needs project-specific data files:
...
Run `felix setup` in a new project directory to scaffold these files automatically.

### Env Var Contract

The C# runner injects two env vars into every PS subprocess:

| Variable             | Purpose                                          | Fallback (direct PS call)          |
| -------------------- | ------------------------------------------------ | ---------------------------------- |
| `FELIX_INSTALL_DIR`  | Path to engine scripts (`core/`, `commands/`, …) | `$PSScriptRoot`                    |
| `FELIX_PROJECT_ROOT` | User's project CWD                               | `Split-Path -Parent $PSScriptRoot` |

This means direct PS invocation (`.\.felix\felix.ps1 run S-0001`) continues to work unchanged — the scripts detect the absence of the env vars and fall back to PSScriptRoot-relative paths.
...
└────────────┬────────────────────────────────────────────────────┘
             ...
             │   └─► Load config.json and local agents.json overrides
             │
             ├─► initialization.ps1
...
- `Get-AgentConfiguration()` - Resolves agent configuration and applies adapter defaults
...
Agent overrides for the local CLI (stored in .felix/agents.json in the repo). Adapters provide
defaults for `executable`, `model`, `working_directory`, and `environment`; overrides are optional.
Backend agent profiles are stored in the database.

- **felix-primary** - droid exec
- **codex-cli** - codex CLI with workspace-write

**Agent Key Generation (Content-Addressed IDs)**

Agent IDs are **deterministically generated** from the agent configuration, not random UUIDs. This ensures:

- **Idempotency**: Same provider + model + settings always produces the same key
- **Immutability**: Changes to agent config automatically change the key
- **Server sync**: Safe to resend configurations without FK constraint violations
- **Auditability**: Key format is `ag_xxxxxxxxx` (12 chars total)

Key components (content-addressed):

```
ag_<9-char-sha256-hash> = sha256(provider || model || settings || machine || project)
```

Example: `ag_a32872067`

This approach replaces random UUID generation and prevents issues where users accidentally editing `agents.json` would create orphaned agent records on the server.

### requirements.json

Slim requirement index — only the fields the agent loop needs to select and track work:

| Field                | Description                                     |
| -------------------- | ----------------------------------------------- |
| `id`                 | Requirement code (e.g. `S-0001`)                |
| `title`              | Short description                               |
| `spec_path`          | Path to the `.md` spec file                     |
| `status`             | Current status (see values below)               |
| `commit_on_complete` | Whether to git-commit when done (default: true) |

Rich metadata (`priority`, `tags`, `depends_on`) is stored in per-spec `.meta.json` sidecars next to each spec file. These are gitignored in remote/team mode and downloaded via `felix spec pull`.
...
On exit codes 2 and 3 in remote mode, `felix-loop.ps1` automatically calls `POST /api/sync/work/release` so the requirement returns to `planned` / `assigned_to = NULL` for another agent to claim.

## Operating Modes

### Local Mode (default)

`sync.enabled = false` in `.felix/config.json`. No network calls. `felix-loop.ps1` reads `requirements.json` directly — existing `in_progress` first, then `planned`, sorted by id. Rich metadata lives inline in `requirements.json`.

### Remote / Team Mode

`sync.enabled = true`. Enables server-side work allocation:

| What                 | How                                                                                |
| -------------------- | ---------------------------------------------------------------------------------- |
| Claim next work      | `GET /api/sync/work/next` — atomic `FOR UPDATE SKIP LOCKED`, returns 204 when idle |
| Release blocked work | `POST /api/sync/work/release` — resets to `planned` + `assigned_to = NULL`         |
| Spec metadata        | `felix spec pull` downloads `.meta.json` sidecars (`priority, tags, depends_on`)   |
| requirements.json    | Slim index only — rich metadata is in `specs/*.meta.json` (gitignored)             |

`core/work-selector.ps1` provides `Get-NextRequirement` (dispatcher) and `Send-WorkRelease`. The loop in `felix-loop.ps1` reads `$LoopConfig.sync.enabled` to choose the mode; env vars `FELIX_SYNC_ENABLED`, `FELIX_SYNC_URL`, `FELIX_SYNC_KEY` override config.
