...
- [ ] Registration endpoint works: `curl -X POST http://localhost:3000/auth/register -d '{"username":"test","password":"pass123"}'` (status 201)
- [ ] Login returns token: `curl -X POST http://localhost:3000/auth/login -d '{"username":"test","password":"pass123"}'` (status 200, returns token)
- [ ] Protected endpoints require auth: `curl http://localhost:3000/api/protected` (status 401)
...
- Create **auth/** module with:
  - **auth/router.py** - Routes for registration, login, password reset
...
- **Always** include the YAML frontmatter block (`---` delimited) at the top of the spec before `# S-NNNN`
