...
try {
    $version = psql -U $POSTGRES_USER -c "SELECT version();" 2>&1
    ...
    Write-Host "Start it with: pg_ctl.exe -D C:\dev\postgres\pgsql\data start" -ForegroundColor Yellow
    exit 1
}
...
if ($Force) {
    ...
    psql -U $POSTGRES_USER -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname = '$DATABASE_NAME';" 2>&1 | Out-Null
    
    # Drop database
    psql -U $POSTGRES_USER -c "DROP DATABASE IF EXISTS $DATABASE_NAME;" 2>&1 | Out-Null
...
$dbExists = psql -U $POSTGRES_USER -tAc "SELECT 1 FROM pg_database WHERE datname = '$DATABASE_NAME';" 2>&1

if ($dbExists -eq "1") {
    Write-Host "[OK] Database '$DATABASE_NAME' already exists" -ForegroundColor Green
}
else {
    psql -U $POSTGRES_USER -c "CREATE DATABASE $DATABASE_NAME;" 2>&1 | Out-Null
...
psql -U $POSTGRES_USER -d $DATABASE_NAME -c $createMigrationsTable 2>&1 | Out-Null
if ($LASTEXITCODE -eq 0) {
...
$appliedMigrations = psql -U $POSTGRES_USER -d $DATABASE_NAME -tAc "SELECT version FROM schema_migrations ORDER BY version;" 2>&1
$appliedSet = @{}
...
foreach ($file in $migrationFiles) {
    ...
    # Run the migration
    psql -U $POSTGRES_USER -d $DATABASE_NAME -f $file.FullName 2>&1 | Out-Null
    ...
    psql -U $POSTGRES_USER -d $DATABASE_NAME -c $insertSql 2>&1 | Out-Null
...
$tables = psql -U $POSTGRES_USER -d $DATABASE_NAME -tAc "\dt" 2>&1 | Select-String -Pattern "^\s*public\s+\|\s+(\w+)" | ForEach-Object { $_.Matches.Groups[1].Value }
