...
function Test-ServerCommand {
    ...
    # Check for python main.py patterns (likely a server start)
    if ($cmd -like "*python*" -and $cmd -like "*main.py*") {
        return $true
    }
    
    # Check for python server patterns
    if ($cmd -like "*python*" -and $cmd -like "*serve*" -and $cmd -notlike "*test*") {
        return $true
    }
...
function Get-TagBasedCommands {
    ...
    if ($Tags -contains "backend") {
        # Look for a tests directory in the project root
