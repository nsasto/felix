...
- **PowerShell 5.1+** or **PowerShell Core 7+** for agent
- **PostgreSQL** for database (required for cloud mode, optional for single-machine)
- **LLM API Access**: Factory (Droid), Anthropic (Claude), OpenAI (Codex), or Google (Gemini)
- **Windows** for tray (CLI works on all platforms)
...
**3. Install Tray on Each Machine**

```bash
# On each team member's machine or VM
cd app/tray-manager
dotnet run

# Configure cloud mode in tray settings:
# - Backend URL: https://your-felix-backend.com (or http://localhost:8080 for local)
...
- Tray automatically registers agent on startup
- Agent appears in web dashboard immediately
- Verify heartbeat active (green status)
- Repeat on all machines to build agent pool

**5. Orchestrate From Dashboard**

- Open web UI: http://localhost:3000
- Navigate to Agent Dashboard
- See all registered agents with status
- Create run and assign to any agent
- Or let `felix loop` auto-distribute work

### Installation

```powershell
# Install CLI to PATH (on each machine or just dev machines)
.\scripts\install-cli.ps1

# Use shorter commands
felix run S-0001
felix loop --max-iterations 100
felix tui
```

### Cloud Deployment (Optional)

- Deploy backend to fly.io, Railway, or any Docker host
- Configure Supabase for auth and database
- Update tray settings on all machines with production backend URL
- See `Enhancements/CLOUD_MIGRATION_SPECS.md` for detailed specs
...
- **[CLOUD_ORCHESTRATION.md](Enhancements/CLOUD_ORCHESTRATION.md)** - Distributed orchestration design
- **[CLOUD_MIGRATION_SPECS.md](Enhancements/CLOUD_MIGRATION_SPECS.md)** - Cloud deployment specs
- **[ARCHITECTURE.md](Enhancements/ARCHITECTURE.md)** - System architecture

### Tutorials

- **[EXECUTION_FLOW.md](tuts/EXECUTION_FLOW.md)** - How Felix executes requirements
- **[FELIX_EXPLAINED.md](tuts/FELIX_EXPLAINED.md)** - Core concepts (Ralph explained)
...
- **Single-Machine**: Run on your local workstation only, no team coordination

Felix is **all three solved**: Autonomous execution with production-grade scaffolding and distributed orchestration.

**The Felix Promise:**

_When you wake up tomorrow, the features you specified today will be implemented, tested, and committed across your entire agent swarm—or you'll know exactly why they weren't._

No surprises. No magic. No single-machine bottlenecks. Just reliable, validated, autonomous software delivery at scale.
...
See [HOW_TO_USE.md](HOW_TO_USE.md) for complete setup instructions.
